...
def next_business_days(last_date, count=20):
    dates = []
...
def calculate_month_forecast(data):
    ...
    mean_120 = safe_float(returns.tail(120).mean(), 0)
    ...
    if ma60_now is not None and ma60_previous not in (None, 0):
        ma60_slope = ma60_now / ma60_previous - 1

    if ma120_now is not None and ma120_previous not in (None, 0):
        ma120_slope = ma120_now / ma120_previous - 1
    ...
    if rsi >= 75:
        rsi_component = -0.0015
    elif rsi >= 65:
        rsi_component = -0.0007
    elif rsi <= 25:
        rsi_component = 0.0015
    elif rsi <= 35:
        rsi_component = 0.0007
    ...
    return {
        ...
        "expected_daily_return": expected_daily_return * 100,
        "month_expected_price": safe_float(
            final_row["기준 예상가"]
        ),
        "month_expected_return": month_return,
        "month_lower": safe_float(
            final_row["예상 하단"]
        ),
        "month_upper": safe_float(
            final_row["예상 상단"]
        ),
        ...
        "mean_20": mean_20 * 100,
        "mean_60": mean_60 * 100,
        "mean_120": mean_120 * 100,
        "ma20_slope": ma20_slope * 100,
        "ma60_slope": ma60_slope * 100,
        "ma120_slope": ma120_slope * 100,
        "rsi": rsi,
        "macd": macd,
...
def create_month_forecast_chart(
    data,
    forecast,
    stock_name,
):
    recent = data.tail(150).copy()
    ...
    figure.update_layout(
        title=f"{stock_name} 다음 20거래일 예상 경로",
        xaxis_title="날짜",
        yaxis_title="가격",
        height=620,
        hovermode="x unified",
...
def create_factor_chart(forecast):
    factors = {
        ...
        "최근 120일 평균수익률": forecast["mean_120"],
        "20일선 기울기": forecast["ma20_slope"],
        "60일선 기울기": forecast["ma60_slope"],
        "120일선 기울기": forecast["ma120_slope"],
    }
...
def create_explanation(forecast):
    ...
    if forecast["mean_20"] > 0:
        messages.append(
            "최근 20거래일 평균 수익률은 상승 방향입니다."
        )
    else:
        messages.append(
            "최근 20거래일 평균 수익률은 하락 방향입니다."
        )

    if forecast["ma60_slope"] > 0:
        messages.append(
            "60일 이동평균선의 기울기는 상승 방향입니다."
        )
    else:
        messages.append(
            "60일 이동평균선의 기울기는 하락 방향입니다."
        )

    if forecast["ma120_slope"] > 0:
        messages.append(
            "120일 이동평균선도 상승 방향이라 장기 추세 요인은 긍정적입니다."
        )
    else:
        messages.append(
            "120일 이동평균선은 하락 방향이라 장기 추세 확인이 필요합니다."
        )
    ...
    messages.append(
        f"최근 60일 일간 변동성은 {forecast['daily_volatility']:.2f}%이며, "
        "20거래일 동안의 불확실성을 반영해 예상 범위를 넓혔습니다."
    )

    return " ".join(messages)


def backtest_month_forecast(data, test_count=30):
    rows = []
    start_position = max(
        140,
        len(data) - test_count - 20,
    )
        ...
        rows.append(
            {
                ...
                "20거래일 예상 수익률(%)": predicted_return,
                "실제 20거래일 수익률(%)": actual_return,
                "방향 적중": (
                    predicted_direction
                    == actual_direction
                ),
...
st.markdown(
    '<div class="main-title">🗓️ 1달 뒤 주가 예상</div>',
    unsafe_allow_html=True,
)
...
st.markdown(
    """
    <div class="subtitle">
        최근 중장기 수익률, 이동평균선, RSI, MACD 및 변동성을 이용해
        다음 20거래일의 기준 예상 경로와 가격 범위를 계산합니다.
    </div>
    """,
    unsafe_allow_html=True,
)
...
with st.spinner(
    f"{selected_name} 데이터를 불러오고 20거래일 예상치를 계산하는 중입니다."
):
...
st.caption(
    ...
    f"20거래일 뒤 예상일: {final_date.strftime('%Y-%m-%d')}"
)
...
metric_columns[1].metric(
    "20거래일 기준 예상가",
    format_price(
        forecast["month_expected_price"],
        selected_currency,
    ),
    f"{forecast['month_expected_return']:+.2f}%",
)

metric_columns[2].metric(
    "20거래일 예상 하단",
    format_price(
        forecast["month_lower"],
        selected_currency,
    ),
)
...
st.markdown(
    ...
    <div class="forecast-card">
        ...
        <div>
            20거래일 기준 예상가:
            <strong>
                {format_price(forecast['month_expected_price'], selected_currency)}
            </strong>
            ({forecast['month_expected_return']:+.2f}%)
        </div>
        <div>
            예상 범위:
            <strong>
                {format_price(forecast['month_lower'], selected_currency)}
                ~
                {format_price(forecast['month_upper'], selected_currency)}
            </strong>
        </div>
    </div>
    """,
...
with path_tab:
    ...
    st.info(
        "예상 경로는 현재의 평균 모멘텀이 이어진다는 단순 가정으로 계산됩니다. "
        "20거래일 동안 뉴스, 실적 및 시장 상황에 따라 실제 경로가 크게 달라질 수 있습니다."
    )


with validation_tab:
    st.subheader("과거 20거래일 예측 간이 검증")

    validation_data = backtest_month_forecast(
        stock_data,
        test_count=30,
    )
    ...
    if validation_data.empty:
        ...
        st.caption(
            "과거 검증 성과는 미래의 예측 정확도를 보장하지 않습니다."
        )
...
st.warning(
    "이 페이지는 1달 뒤 주가를 확정적으로 예측하지 않습니다. "
    "예상가는 과거 가격의 모멘텀과 변동성만 이용한 통계적 참고치입니다. "
    "뉴스, 실적 발표, 금리, 환율, 정책, 지정학적 사건 및 수급 변화는 반영하지 못합니다."
)
